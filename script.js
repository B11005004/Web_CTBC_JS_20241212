...
// 判斷式語法
// if(布林值){布林值等於true執行此區塊block;false則不執行}
if (true) {
    console.log("這裡是判斷式");
}
...
// console.log("表達函式：",Math.floor(bmiExpression(60,1.68)));
...
console.log(Math.floor(bmi(60,1.68))); //Math.floor可以去除小數點
// 表達函式
const bmiExpression = function(weight,height){
    return weight/ square(height);
}
console.log(Math.floor(bmiExpression(60,1.68)));
console.log(Math.floor(bmiExpression(70,1.85)));
// 箭頭函式
const bmiArrow =(weight,height) =>{
    return weight/square(height);
}
console.log(Math.floor(bmiArrow(70,1.85)));
console.log(Math.floor(bmiArrow(90,1.36)));